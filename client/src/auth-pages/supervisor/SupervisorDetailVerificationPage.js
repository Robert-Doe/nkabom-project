...
import style from './SupervisorDetailVerificationPage.module.css'
import axios from "axios";
import {Navigate, useNavigate} from "react-router-dom";
import OpaqueNav from "../../pages/reusables/OpaqueNav";

function SupervisorDetailVerificationPage() {
        ...
        axios.get(`http://localhost:9999/api/supervisors/${staffId}`)
            .then(response => {
                if (response.status === 200) {
                    ...
                    fNameRef.current.value = supervisor.firstName
                    sNameRef.current.value = supervisor.lastName
                    dobRef.current.value = supervisor.dob.toString().substr(0, 10)
                    phoneRef.current.value = supervisor.phone
                    emailRef.current.value = supervisor.email
                    // passKeyRef.current.value=intern.keyHash
                    addressKeyRef.current.value = supervisor.address
                    bioRef.current.value = supervisor.bio

                    console.log(supervisor)// use the navigate function to redirect to the login page
                    // navigate('/');
                }
            })
    }
...
export default SupervisorDetailVerificationPage;